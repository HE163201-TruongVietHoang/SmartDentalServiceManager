...
function UsersTable({ onRefresh }) {
  ...
  async function fetchUsers() {
    try {
      ...
      let q = `?page=${page}&pageSize=${pageSize}&search=${encodeURIComponent(
        search
      )}`;
      ...
      if (res.ok) {
        ...
        toast.error(data.error || "Lỗi tải danh sách người dùng");
      }
    } catch (err) {
  ...
  async function doDelete(userId) {
    ...
    if (res.ok) {
      ...
      toast.error(data.error || "Lỗi xoá người dùng");
    }
  }
  ...
  async function toggleActive(userId, isActive) {
    ...
    const res = await fetch(
      `http://localhost:5000/api/auth/users/${userId}/active`,
      {
        method: "PATCH",
        headers: {
  ...
  async function handleEdit(userId) {
    ...
    if (res.ok) {
      ...
      toast.error(user.error || "Lỗi tải thông tin người dùng");
    }
  }
  ...
  async function handleSaveEdit(formData) {
    ...
    const res = await fetch(
      `http://localhost:5000/api/auth/users/${editingUser.userId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
